...
///
/// The driver-side mirror of the passenger app's model. Kept as a separate file
/// rather than a shared package because the two apps are independent Flutter
/// projects with no shared local package — but the wire contract, the stage
/// vocabulary and the dedupe rule are deliberately identical, and both are
/// generated from the same backend contract (see
/// docs/ride_coordination_contract.md).
///
...
/// What the driver is being told. Coarser than the backend's nine operational
/// states — support needs to distinguish `driver_offline` from
/// `passenger_offline`, a driver needs to know whether the passenger is coming out.
enum CoordinationStage {
  /// Ordinary ride. No coordination card is shown.
  none,

  /// The driver is running late; nobody has been asked anything yet.
  runningLate,
  ...
  /// The driver has arrived and is waiting for the passenger to come out.
  waitingForPassenger,
...
enum CoordinationAction {
  ...
  /// The label the driver reads. Plain language; no "stale", no "timeout".
  String get label => switch (this) {
        CoordinationAction.stillComing => "I'm still coming",
        CoordinationAction.keepWaiting => 'Keep waiting',
        CoordinationAction.onMyWay => "I'm on my way",
        CoordinationAction.acceptCancellation => 'Accept cancellation',
        CoordinationAction.continueRide => "I'm still coming",
        CoordinationAction.requestCancel => 'Cancel ride',
        CoordinationAction.findAnotherDriver => 'Cancel ride',
        CoordinationAction.callOtherParty => 'Call passenger',
        CoordinationAction.messageOtherParty => 'Message passenger',
...
class RideCoordination {
  ...
  /// whether the answer on record was this driver's own.
...
enum RideClosure {
  ...
  /// The action offered alongside the closing message. A driver's way forward is
  /// getting back on the road, not booking a ride.
  String get primaryAction => 'Go back online';
}
